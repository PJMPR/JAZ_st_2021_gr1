...
public class RulePaging extends RulesBase {
    ...
    private static void setItems(Results results, SearchParameters params) {
        results.setItems(getPersonsStream(results).skip(getCurrentSkip(results, params))
                .limit(params.getPage().getSize()).toList());
    }
