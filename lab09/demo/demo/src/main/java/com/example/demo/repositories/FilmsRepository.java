...
import com.example.demo.exceptions.NoSuchPageException;
...
public class FilmsRepository {
    ...
    public List<FilmDto> getFilmsByPage(int page, int size,FilmDto film) {
        ...
        try {
            return (List<FilmDto>) entityManager.createNativeQuery(query+ " limit " +size+ " offset "
    ...
    public HttpStatus createFilm(FilmDto newFilm) {
        try {
            ...
            entityManager.createNativeQuery("INSERT INTO Film " + "(title,release_year,rental_duration,rental_rate,replacement_cost,last_update,language_id)"
                            + "VALUES (?,?,?,?,?,?,?)")
